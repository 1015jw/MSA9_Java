...
public class BoardAccess implements BoardInterface {
	...
	@Override
	public Board[] list() {
		if( this.count == 0 || boardList.length == 0) {
	 ...
	 * 2. 수정할 게시글 번호 no가 유효하면, 번호에 해당하는 boardList[] 요소를 변경
	...
	public int update(Board board) {
		...
		boardList[no-1] = oldBoard;				// 1 순서가 바뀌어도 무방 (참조 자료형)
		oldBoard.setUpdDate("2024/08/20 - 12:00");	// 2
		return 1;
	}
