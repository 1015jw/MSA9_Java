-- Active: 1725516947037@@127.0.0.1@1521@orcl@JOEUN
-- 72. 1)
...
-- 외래키 : MS_BOARD(WRITER) ----> MS_USER(USER_no)
ALTER TABLE MS_BOARD ADD CONSTRAINT MS_BOARD_WRITER_FK
FOREIGN KEY (WRITER) REFERENCES MS_USER(USER_NO);


-- 72. 2) 외래키 지정 : MS_FILE ( BOARD_NO ) ----> MS_BOARD (BOARD_NO)
-- MS_FILE 테이블의 BOARD_NO를 외래키로 지정하여, MS_BOARD 테이블의 BOARD_NO를 참조하도록 지정
...
SELECT * FROM MS_FILE;
INSERT INTO MS_FILE
VALUES(2, 1, '고양이.webp', '123', '2024/09/05', '2024/09/05', 'webp');
...
-- * RESTRICT       :   자식 테이블 데이터가 존재하면 삭제 안 함.
...
FROM EMPLOYEE e, DEPARTMENT d
WHERE e.DEPT_CODE = d.DEPT_ID
    AND e.SALARY > 3700000
;

--3) 3700000 자리에 1)번 쿼리 대입
SELECT e.EMP_ID 사원번호
      ,e.EMP_NAME 직원명
      ,d.DEPT_ID 부서번호
      ,d.DEPT_TITLE 부서명
...
-- 1.
-- MANAGER_ID 컬럼이 NELL 아닌 사원을 중복 없이 조회
-- → 매니저들의 사원 번호
...
-- 3.
-- 조인 결과 중, EMP_ID가 매니저 사원번호인 경우만 조회
SELECT E.EMP_ID 사원번호
      ,E.EMP_NAME 직원명
      ,D.DEPT_TITLE 부서명
      ,JOB_NAME 직급명
...
SELECT E.EMP_ID 사원번호
     ...
     ,
     -- 나이
     -- 1900년대 출생 (주민번호 뒷자리 첫 글자 1,2)
     -- 2000년대 출생 (주민번호 뒷자리 첫 글자 3,4)
     TRUNC(
            MONTHS_BETWEEN(
                    SYSDATE,
                    TO_DATE(
...
SELECT ROWNUM 순번
      ...
      END AS 구분
      ,CASE 
          WHEN SUBSTR(E.EMP_NO, 8, 1) IN ('1', '3') THEN '남자'
          WHEN SUBSTR(E.EMP_NO, 8, 1) IN ('2', '4') THEN '여자'
          ELSE '알수없음'  -- 성별 정보가 없거나 잘못된 경우 처리
      END AS 성별
    ,TO_NUMBER(
            EXTRACT(YEAR FROM SYSDATE) - 
            (CASE 
                WHEN SUBSTR(E.EMP_NO, 8, 1) IN ('1', '2') THEN '19'
                WHEN SUBSTR(E.EMP_NO, 8, 1) IN ('3', '4') THEN '20'
      ...
      ,RPAD(SUBSTR(E.EMP_NO, 1, 8), 14, '*') AS 주민등록번호
      ,TO_CHAR(E.HIRE_DATE, 'YYYY.MM.DD') 입사일자
      -- 연봉 = (급여 + (급여*보너스)) * 12
      ,TO_CHAR(
         (E.SALARY + (SALARY * NVL(BONUS, 0))) * 12
         ,
         '999,999,999,999'
      ) 연봉
FROM EMPLOYEE E
...
SELECT SEQ_MS_BOARD.nextval FROM dual;
SELECT SEQ_MS_FILE.nextval FROM dual;
SELECT SEQ_MS_REPLY.nextval FROM dual;
...
-- ROWS BETWEEN UNBOㅕNDED PRECEDING AND UNBOUNDED FOLLOWING
-- 현재 행을 기준으로 첫 번째 행부터 마지막 행까지 모두 포함하여 계산
...
WHERE row_num >= 21; -- 원하는 페이지 번호에 맞게 시작하는 행 번호를 여기에 지정합니다.
...
-- 변수 선언과 출력
SET SERVEROUTPUT ON;         -- 실행결과 출력하도록 설정 (SQL PLUS, SQL Developer에서 실행 가능)

DECLARE
        VI_NUM  NUMBER;      -- 선언부 (NEMBER 타입 변수 VI_NUM 선언)
BEGIN
        VI_NUM := 100;       -- 실행부
...
-- 조회하여 1,000,000원 이상 2,000,000 이하이면 1 출력
--         2,000,001원 이상, 3,000,000 이하이면 2 출력
--          그렇지 않으면 3을 출력해보세요
...
BEGIN
        ...
        -- 조건문
        IF MAX_SALARY BETWEEN 10000000 AND 2000000 THEN
...
BEGIN
        ...
        -- 조건문
        CASE
            WHEN MAX_SALARY BETWEEN 10000000 AND 2000000 THEN
                 DBMS_OUTPUT.PUT_LINE('1');
...
-- 114.
-- 사원번호로 부서명을 구하는 함수를 정의하는 PL/SQL을 작성하시오.
-- 함수명 : dept_title
...
-- 프로시저 실행
-- 200QJS 사원의 직무를 IT_PROG로 변경하고, 직무이력 해당 기간으로 갱신하시오.
EXECUTE PRO_APP_EMP('200','IT_PROG','2025/03/02','2027/03/02');
