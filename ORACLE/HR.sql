-- Active: 1725502740660@@127.0.0.1@1521@orcl@ALOHA
...
-- 4. employees 테이블의 employee_id, first_name 조회
...
-- 6.
-- 테이블 EMPLOYEES의 SALARY(급여)가 6000을 초과하는 사원의 모든 칼럼을
-- 조회하는 SQL문을 작성하시오.
SELECT *
FROM employees
WHERE salary > 6000;
...
-- 7.
-- 테이블 EMPLOYEES의 SALARY(급여)가 10000인 사원의 모든 칼럼을
-- 조회하는 SQL문을 작성하시오.
SELECT *
FROM employees
WHERE salary = 10000;
...
-- 9.
-- 테이블 EMPLOYEES의 JOB_ID가 'FT_ACCOUNT'이거나 'IT_PROG'인
...
-- 11.
-- 테이블 EMPLOYEES의 JOB_ID가 'FT_ACCOUNT'이거나 'IT_PROG'이 아닌
-- 사원의 모든 컬럼을 조회하는 SQL문을 작성하시오.
-- 조건 연상
-- IN 연산 : ~또는, ~이거나
-- WHERE A OR B
...
-- 13.
-- 테이블 EMPLOYEES의 FIRST_NAME이 'S'로 시작하는
-- 사원의 모든 컬럼을 조회하는 SQL문을 작성하시오.
SELECT *
FROM employees
...
-- 21
-- 12.45, -12.45보다 크거나 같은 정수 중 제일 작은 수를
-- 계산하는 SQL문을 각각 작성하시오.
-- * dual ? 
...
-- 22
-- 12.55, -12.5보다 작거나 같은 정수 중 제일 큰 수를
-- 계산하는 SQL문을 각각 작성하시오.
...
-- : 해당 문자열에서 'A'를 1번 위치부터 찾아서 2번째 나온 A 위치 반환
...
SELECT first_name AS 이름
      ,TO_CHAR(hire_date, 'YYYY-MM-DD (DY) HH:MI:SS') 입사일자
FROM EMPLOYEES;
...
-- DISTINCT : 중복 없이 조회
-- NVL(값, 대체할 값) : 해당 값이 NULL이면 지정된 값으로 반환하는 함수

-- * ORDER BY의 정렬기준 컬럼은, SLEECT에서 선택한 컬럼만 사용 가능하다
...
SELECT FIRST_NAME 이름
      ...
      ,CASE WHEN DEPARTMENT_ID = 10 THEN 'Administration'
            WHEN DEPARTMENT_ID = 20 THEN 'Marketting'
...
-- '2024/03/01' : 문자타입이 DATE로 내부적으로 변환돼서 데이터 추가
-- DB 툴을 이용하여 INSERT 한다면, COMMIT을 실행해야 LOCK 걸리지 않고 적용됨
